...
color FA
del List Password Wifi\V3\Informasi.txt
del List Password Wifi\V3\FOTGPDhIfRGlrvStQRTg.txt
del List Password Wifi\V3\FOTGPDhIfRGlrvStQRTg_.txt
del List Password Wifi\V3\iMgPhbRoFsIRCgYPmizr_.txt
del List Password Wifi\V3\iMgPhbRoFsIRCgYPmizr.txt
...
md "List Password Wifi\V3" 2>nul
...
> "List Password Wifi\V3\SMP PETRA 3 STUDENT.txt" (
  echo Password = pppkp3tr4
)

> "List Password Wifi\V3\ICT Room1.txt" (
  echo Password = xeqyerSlY
)

> "List Password Wifi\V3\LAB BAHASA.txt" (
  echo Password = lexeqe2155
)

> "List Password Wifi\V3\LibraryP3.txt" (
  echo Password = xaqxzsx3
)
start explorer "%userprofile%\Desktop\List Password Wifi\V3"
del List Password Wifi\V3\Informasi.txt
del List Password Wifi\V3\FOTGPDhIfRGlrvStQRTg.txt
del List Password Wifi\V3\FOTGPDhIfRGlrvStQRTg_.txt
del List Password Wifi\V3\iMgPhbRoFsIRCgYPmizr_.txt
...
powershell -command "& { (Get-Content 'List Password Wifi\V3\ICT Room1.txt' -Raw) -replace 'x', 'M' -replace 'e', 'a' -replace 'q', 'n' -replace 'l', 'B' -replace 'Loading...' , ' '  | Set-Content 'List Password Wifi\V3\ICT Room1.txt' }"
powershell -command "& { (Get-Content 'List Password Wifi\V3\LAB BAHASA.txt' -Raw) -replace 'x', 'h' -replace 'e', 'a' -replace 'q', 's' -replace 'l', 'b' -replace 'Loading...' , ' ' | Set-Content 'List Password Wifi\V3\LAB BAHASA.txt' }"
powershell -command "& { (Get-Content 'List Password Wifi\V3\LibraryP3.txt' -Raw) -replace 'x', 'p' -replace 'a', 'e' -replace 'z', 'u' -replace 'q', 'r' -replace 'Loading...' , ' '  | Set-Content 'List Password Wifi\V3\LibraryP3.txt' }"